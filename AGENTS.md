...
- Children receive `hass`, `room` and `active` as plain attribute bindings (`hass=${...}`, not `.hass=`/`?active=`) with untyped `static properties`. This is how the recovered bundle runs on the live Home Assistant panel and is not a defect; do not convert to property bindings.
