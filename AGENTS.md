...
- `nspanel-card` owns `activeTab` (`music` | `water`); it swaps the `nspanel-button-card` grid contents, the whole bottom section, and the background (the split gradient exists for the album art; the water screen is flat). Water entities are hardcoded in `src/card.js` (switches, `script.lawn_{10,20,30}_minutes`) and `src/components/water-status.js` (sensors), the same way the music screen hardcodes `media_player.amplifier`; rooms come from card config. A `room` with `status`/`detail` set renders those in place of the on/off text.
- Known defects are deliberately preserved from the recovered build (inverted shuffle/repeat icons); see the recovery PR (#1) for the list. Do not "fix" them in passing without the captain's decision.
